...
즉, WP_Query는 get_posts()로만 wp_posts(table) 조회하지 않고 parse_request()로도  
wp_posts(table) 조회해서 Rewrite rule(for page or post) 구분한다.  
...
※ 내부에서 get_option()로 wp_options(table) 조회 후  
"is_front_page", "is_posts_page" 설정.  
...
# Reading Settings와 Front / Posts Page 분기  
